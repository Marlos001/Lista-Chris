...
se idade < 16 && > 18 == true:
voto facultativo == true
voto obrigatório == false
______
...
Classe Circulo extends FormaGeometrica:
    ...
    Metodo Constructor(cor, raio):
        Define o valor do atributos com os valores passados como parâmetro.
...
função CalculoDeTempo(velocidadeInicial, aceleracao, distanciaTotal, velocidadeMaxima, tempoMaximo):
    tempo = 0
    distanciaPercorrida = 0
    velocidadeAtual = velocidadeInicial

    enquanto distanciaPercorrida < distanciaTotal e tempo <= tempoMaximo:
        se velocidadeAtual < velocidadeMaxima:
            velocidadeAtual = velocidadeInicial + aceleracao * tempo
        senão:
            velocidadeAtual = velocidadeMaxima
        
        distanciaPercorrida = distanciaPercorrida + velocidadeAtual / 60
        tempo = tempo + 1

        se distanciaPercorrida >= distanciaTotal:
            imprima "A corrida foi completada em ", tempo, " minutos."
            retorna
...
Função MultiplicacaoDeMatrizes(matrizA, matrizB):
    Se tamanho(matrizA[0]) ≠ tamanho(matrizB) então:
        Retornar "As matrizes não podem ser multiplicadas. Número de colunas da primeira matriz é diferente do número de linhas da segunda."
    Senão:
